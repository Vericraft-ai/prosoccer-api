...
# Important Commands
- `npm run compile` - Compile code, after writing a new contract / changing existing contract
- `npx hardhat --network [networkName] deploy --tags Prosoccer` - Deploy (and sometimes verify) contract with tag `Prosoccer` to selected network. (Check `hardhat.config.ts` for network names)
- `npx hardhat run scripts/mint.ts --network [networkName]` - Run a script to mint and list tokens
- `npx hardhat run scripts/buy.ts --network [networkName]` - Run a script to buy tokens

You can add more scripts to run smart contart functions in `scripts` folder and run them using `npx hardhat run scripts/[scriptName].ts --network [networkName]` command. You can also interact with deployed contracts using at the explorer address, e.g, for [PolygonAmoy](https://amoy.polygonscan.com/address/0xd8D2631FFf643eA60E8954EAD9f64f6c6b67da86)
